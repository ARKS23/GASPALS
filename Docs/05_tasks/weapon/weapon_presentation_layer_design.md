...
`UWeaponPresentationComponent` 已监听成功射击事件，并在 Overlay Mesh 的 `Muzzle` socket 播放枪口特效和开火音效。
...
-> OverlaySkeletalMesh.Muzzle VFX / Fire Sound
```
...
- `WeaponPresentationComponent`：Muzzle VFX、Fire Sound 和后续 Impact/Tracer。
...
Fire Sound 已通过 `OnWeaponShot` 迁移到 `WeaponPresentationComponent`，`WeaponBase::PlayFireFeedback()` 的旧调用、声明和实现均已删除。DryFire/Reload Sound 暂时保留在 `WeaponBase`，后续分别通过对应事件迁移。
...
| 3 | BP_PlayerCharacter 接入 | 已完成 |
| 4 | DA_Rifle 配置并清理 BP_Rifle 旧通用 VFX 逻辑 | 已完成 |
| 5 | Fire Sound 迁移 | 已完成 |
...
- [x] 项目 C++ 编译通过。
- [x] `BP_PlayerCharacter`、`BP_Rifle` 编译无错误。
- [x] 按 1 装备后持枪姿势和手部 IK 不回归。
- [x] 半自动每发只有一个 Muzzle VFX。
...
- [x] `BP_Rifle` 不再 Cast `BP_PlayerCharacter` 播放通用枪口特效。
- [x] 每次成功射击只播放一次 Fire Sound。
